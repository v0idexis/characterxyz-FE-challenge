...
let watchDrag: boolean = true;
if (typeof window !== 'undefined') {
    windowWidth = window.innerWidth >= 767 ? 3 : 1
    watchDrag = window.innerWidth >= 767 ? false : true;
}
const options: EmblaOptionsType = { loop: false, slidesToScroll: windowWidth, duration: 40, startIndex: 0, watchDrag: watchDrag, align: 'start' } //containScroll: 'trimSnaps',
                    ...
                    <div className="embla relative items-center w-full" >
                        <div className="embla__viewport relative items-center w-full" ref={emblaRef}>
                            <div className='embla__container flex gap-4'>
                                {
                                    moviesArray.map((item: MoviesObject, index: number) => {
                                        {
                                            return (
                                                <Fragment key={hashStr(item.backdrop_path)}>
                                                    <div className="embla__slide w-[200px] max-w-[200px] rounded-lg relative ease-in-out duration-150 overflow-hidden flex flex-row items-center" >
                                                        <div className="absolute text-[100px]/[100px] font-bold text-white z-20 top-2 left-2 drop-shadow-sm">{index + 1}</div>
